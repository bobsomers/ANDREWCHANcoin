...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "ANDREWCHANcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    // ANDREWCHANcoin TODO: Actually run some DNS seeds?
...
static const char *strTestNetDNSSeed[][2] = {
    // ANDREWCHANcoin TODO: Actually run some testnet DNS seeds?
...
unsigned int pnSeed[] =
{
    // ANDREWCHANcoin: Add more seed nodes?
    // (Convert IP octets to hex and swap byte order.)
    0x6ddfaf42 // 66.175.223.109 adama.bobsomers.com
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. ANDREWCHANcoin is probably already running."), addrBind.ToString().c_str());
        else
